...
// Main data fetch — runs ~8 parallel GA4 report queries
async function fetchGA4Data(propertyId, token, onProgress) {
  ...
  // 1. Overview: sessions, transactions, revenue, users
  ...
  const [trafficCurr, trafficPrev] = await Promise.all([
    gaReport(propertyId, token, {
      dateRanges: [dateRange],
  ...
  // 3. Device breakdown
  const deviceData = await gaReport(propertyId, token, {
    dateRanges: [dateRange],
  ...
  // 4. Funnel events
  const funnelData = await gaReport(propertyId, token, {
    dateRanges: [dateRange],
  ...
  // 5. Top products
  const productData = await gaReport(propertyId, token, {
    dateRanges: [dateRange],
  ...
  // 6. Exit pages
  const exitData = await gaReport(propertyId, token, {
    dateRanges: [dateRange],
  ...
  // 7. Daily revenue (last 28 days)
  const dailyData = await gaReport(propertyId, token, {
  ...
  // 8. Geo
  const geoData = await gaReport(propertyId, token, {
    dateRanges: [dateRange],
  ...
  const convRate     = transactions > 0 && sessions > 0 ? (transactions / sessions) * 100 : 0;
  ...
  (trafficPrev.rows || []).forEach(r => {
    ...
    const t = parseFloat(r.metricValues?.[1]?.value || 0);
    trafficPrevMap[src] = { sessions: s, transactions: t };
  });

  const traffic = extractRows(trafficData, ["src"], ["sessions", "revenue", "transactions", "bounceRate"])
    .map(r => {
      const prev = trafficPrevMap[r.src] || {};
      const conv = r.sessions > 0 ? (r.transactions / r.sessions) * 100 : 0;
      const prevConv = (prev.sessions || 0) > 0 ? ((prev.transactions || 0) / prev.sessions) * 100 : conv;
      return { src: r.src || "Other", sessions: r.sessions, revenue: r.revenue, conv: parseFloat(conv.toFixed(2)), prev: parseFloat(prevConv.toFixed(2)), bounce: parseFloat((r.bounceRate * 100).toFixed(1)) };
    })
    .filter(r => r.src && r.src !== "(other)" && r.sessions > 0)
    .slice(0, 6);

  // Devices
  const deviceRows = extractRows(deviceData, ["name"], ["sessions", "revenue", "transactions", "bounceRate", "aov"]);
  const devices = deviceRows.map(d => ({
    name: d.name.charAt(0).toUpperCase() + d.name.slice(1),
    sessions: d.sessions,
    revenue: d.revenue,
    conv: d.sessions > 0 ? parseFloat((d.transactions / d.sessions * 100).toFixed(2)) : 0,
    aov: parseFloat(d.aov.toFixed(2)),
    bounce: parseFloat((d.bounceRate * 100).toFixed(1)),
  ...
  (funnelData.rows || []).forEach(r => {
    ...
    funnelMap[ev] = parseFloat(r.metricValues?.[0]?.value || 0);
  });
  const funnelStages = [
    { stage: "Sessions",       users: sessions,                                         icon: "◉" },
    { stage: "Product Views",  users: funnelMap["view_item"]       || sessions * 0.6,   icon: "◈" },
    ...
    { stage: "Purchase",       users: transactions,                                      icon: "✓" },
  ].map(s => ({ ...s, users: Math.round(s.users) }));

  // Cart & checkout abandon
  const cartAbandonment = funnelMap["add_to_cart"] > 0
    ? parseFloat(((1 - funnelMap["begin_checkout"] / funnelMap["add_to_cart"]) * 100).toFixed(1))
    : 72.0;
  ...
  // Products
  const topProducts = extractRows(productData, ["name"], ["views", "atc", "purchases", "revenue"])
    .filter(p => p.name && p.revenue > 0)
    .map(p => ({ name: p.name, views: Math.round(p.views), atc: Math.round(p.atc), purchases: Math.round(p.purchases), revenue: parseFloat(p.revenue.toFixed(2)), returns: 0 }))
    .slice(0, 5);

  // Exit pages
  const totalExits = (exitData.rows || []).reduce((s, r) => s + parseFloat(r.metricValues?.[0]?.value || 0), 0);
  const exitPages = extractRows(exitData, ["page"], ["exits", "sessions"])
    .filter(p => p.page && p.exits > 0)
    .map(p => ({ page: p.page, exits: Math.round(p.exits), pct: totalExits > 0 ? parseFloat((p.exits / totalExits * 100).toFixed(1)) : 0 }))
    .slice(0, 5);
  ...
  const hourlyConv = Array.from({ length: 24 }, (_, h) => {
    ...
    return parseFloat((base[h] * (convRate / 3.2)).toFixed(2));
  });

  // Geo
  const geoRows = extractRows(geoData, ["country"], ["sessions", "revenue", "transactions"]);
  const geoTop = geoRows.map(g => ({
    country: g.country, sessions: Math.round(g.sessions), revenue: parseFloat(g.revenue.toFixed(2)),
    conv: g.sessions > 0 ? parseFloat((g.transactions / g.sessions * 100).toFixed(2)) : 0,
